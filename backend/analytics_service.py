...
class FinancialAnalytics:
    ...
    def analyze_operations(
        ...
        # ترتيب Top Services
        top_services = sorted(
            services_revenue.values(), key=lambda x: x["revenue"], reverse=True
        )[:5]

        # ترتيب Top Parts
        top_parts = sorted(
            parts_revenue.values(), key=lambda x: x["revenue"], reverse=True
        )[:5]
    ...
    def generate_analytics_report(
        ...
        total_liabilities = sum(
            a["balance"] for a in accounts if a["type"] == "liability"
        )
