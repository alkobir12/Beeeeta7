...
class DensoSystemV7:
    ...
    def __init__(self):
        ...
        self.denso_test_sequence = {
            ...
            "step_7": {
                ...
                "tools_required": ["Spray pattern analyzer", "High-speed camera (optional)"],
    ...
    def validate_vl_mode(self, engine_id: str, pressure_bar: float, 
                        duration_us: float, return_qty_ml_min: float) -> Dict:
        """
        ...
        if not pressure_ok:
            messages.append(f"⚠️ الضغط خارج النطاق المقبول ({pressure_range[0]}-{pressure_range[-1]} bar)")
            messages_en.append(f"⚠️ Pressure out of range ({pressure_range[0]}-{pressure_range[-1]} bar)")
        
        if not duration_ok:
            messages.append(f"⚠️ المدة أقل من الحد الأدنى ({vl_params['duration_min_microseconds']} μs)")
            messages_en.append(f"⚠️ Duration below minimum ({vl_params['duration_min_microseconds']} μs)")
        
        if not return_qty_ok:
            messages.append(f"⚠️ كمية الرجوع تتجاوز الحد الأقصى ({vl_params['return_quantity_max_ml_min']} ml/min)")
            messages_en.append(f"⚠️ Return quantity exceeds maximum ({vl_params['return_quantity_max_ml_min']} ml/min)")
    ...
    def generate_full_report(self, engine_id: str, test_data: Dict) -> Dict:
        ...
        if all(k in test_data for k in ['pressure_bar', 'duration_us', 'return_qty_ml_min']):
        ...
        report['overall_status'] = {
            "pass": all_valid,
            "message_ar": "✅ الحاقن يعمل ضمن المواصفات" if all_valid else "❌ الحاقن يحتاج إلى صيانة أو استبدال",
            "message_en": "✅ Injector operating within specifications" if all_valid else "❌ Injector needs service or replacement"
        }
