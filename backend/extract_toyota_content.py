        ...
        for img in soup.find_all('img'):
            ...
            if src:
                images.append({
                    'src': src,
                    'alt': img.get('alt', ''),
                    'path': str(html_path.parent / src) if not src.startswith('http') else src
                })
...
def main():
    ...
    print(f"   - Progress: {len(batch_results)}/{len(html_files)} ({100*len(batch_results)//len(html_files)}%)")
...
if __name__ == '__main__':
    ...
    print(f"   3. Process remaining {stats['total'] - stats['processed']} files in background")
