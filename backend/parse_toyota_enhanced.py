...
def parse_content_file(html_path):
    ...
    try:
        ...
        for h3 in soup.find_all("h3"):
            ...
            if section_text:
                data["content"].append(
                    {"type": "heading", "level": 3, "text": section_text}
                )
        ...
        for ol in soup.find_all("ol"):
            ...
            if items:
                data["procedures"].append({"type": "procedure", "steps": items})

        # Extract paragraphs
        for p in soup.find_all("p"):
            p_text = clean_text(p.get_text())
            if p_text and len(p_text) > 10:
                data["content"].append({"type": "paragraph", "text": p_text})
...
def process_batch_parallel(files, batch_name, max_workers=20):
    ...
    with ThreadPoolExecutor(max_workers=max_workers) as executor:
        ...
        for future in as_completed(future_to_file):
            ...
            if completed % 100 == 0:
                print(
                    f"   Progress: {completed}/{len(files)} files ({100*completed//len(files)}%)"
                )
...
def main():
    ...
    for i in range(0, min(3000, len(html_files)), batch_size):
        ...
        batch_results = process_batch_parallel(
            batch_files, f"batch_{i//batch_size + 1}"
        )
        ...
        # Save batch
        with open(
            OUTPUT_DIR / f"content_batch_{i//batch_size + 1}.json",
            "w",
            encoding="utf-8",
        ) as f:
    ...
    for doc in all_content:
        ...
        search_text = " ".join(
            [
                ...
                " ".join(
                    [" ".join(p.get("steps", [])) for p in doc.get("procedures", [])]
                ),
            ]
        )
