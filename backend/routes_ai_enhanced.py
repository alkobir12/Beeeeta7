...
async def _fallback_local_results(query: str, k: int = 5) -> List[Dict[str, Any]]:
    ...
    if provider == "memory" or db is None:
        try:
            p = os.path.join(
                os.path.dirname(__file__), "uploads", "knowledge_documents.json"
            )
            if os.path.exists(p):
    ...
    try:
        ...
        docs2 = (
            await db.ai_kb_docs.find(q2)
            .limit(max(0, k - len(out)))
            .to_list(length=max(0, k - len(out)))
        )
        for d in docs2:
            d.pop("_id", None)
            out.append(
...
async def _maybe_summarize(query: str, results: List[Dict[str, Any]]) -> Optional[str]:
    try:
        ...
        has_manuals = any(
            r.get("metadata", {}).get("source") == "manual" for r in results
        )
        ...
        chat = LlmChat(
            api_key=key, session_id=str(uuid.uuid4()), system_message=sys
        ).with_model(model_provider, model_name)
        ans = await chat.send_message(
            UserMessage(text=f"سؤال: {query}\n\nالمصادر:\n{ctx}")
        )
...
async def search_history(provider: Optional[str] = None, limit: int = 50):
    try:
        ...
        if provider:
            q["provider"] = provider
        rows = (
            await db.ai_search_logs.find(q)
            .sort("createdAt", -1)
            .limit(limit)
                    ...
                    results.append(
                        {
                            ...
                            "text": nd.node.get_content()[:800],
                            "metadata": nd.node.metadata,
                        }
                    )
            except Exception:
                pass
        ...
        if key:
            async with httpx.AsyncClient(timeout=20) as client:
                ...
                await _log_search(
                    ...
                    ok,
                    "api",
                    len(data.get("web", {}).get("results", [])) if ok else 0,
                )
        ...
        if key:
            async with httpx.AsyncClient(timeout=20) as client:
                r = await client.get(
                    ...
                    params={"q": q, "num_web_results": 5},
                    headers={"X-API-Key": key},
                )
                ok = r.status_code == 200
                data = r.json() if ok else {"status": r.status_code}
                await _log_search(
                    "you", q, ok, "api", len(data.get("hits", [])) if ok else 0
                )
        ...
        if key:
            async with httpx.AsyncClient(timeout=30) as client:
                ...
                await _log_search(
                    ...
                    ok,
                    "api",
                    len(data.get("results", [])) if ok else 0,
                )
