...
async def extract_dtc_cards(payload: Dict[str, Any]):
    ...
    try:
        ...
        dtc_pattern = re.compile(
            r"\b(P[0-9A-F]{4}|U[0-9A-F]{4}|C[0-9A-F]{4}|B[0-9A-F]{4})\b", re.IGNORECASE
        )
...
كن دقيقاً ومحدداً بناءً على السياق المعطى."""
        ...
        return {"cards": dtc_cards[:20], "count": len(dtc_cards), "query": query}
    except Exception as e:
    ...
    for line in lines:
        ...
        if "اسم العطل" in line or "الاسم" in line or "Name" in line.lower():
            ...
            if current_section == "name" and not name:
                name = line_clean
            elif current_section == "causes" and (
                line_clean.startswith("-")
                or line_clean.startswith("•")
                or line_clean.startswith(("1", "2", "3", "4", "5"))
            ):
                causes.append(line_clean.lstrip("-•123456789. "))
            elif current_section == "fixes" and (
                line_clean.startswith("-")
                or line_clean.startswith("•")
                or line_clean.startswith(("1", "2", "3", "4", "5"))
            ):
