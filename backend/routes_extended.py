...
async def confirm_operation_payment(op_id: str, payload: Dict[str, Any] = Body(None)):
    """Confirm payment for a credit operation.

    Business rule:
    - Credit operations (payment_method='credit') are recorded in operations immediately.
    - When cash is actually received/paid, we create a journal entry reflecting cash movement.

    This endpoint creates ONE journal entry:
    - Sales: Dr Cash(101) / Cr AR(113)
    - Purchases: Dr AP(211) / Cr Cash(101)

    Supports partial payments via payload.amount.
    """
    try:
        provider = os.environ.get("DB_PROVIDER", "mongo").lower()
        if provider != "supabase":
        ...
        # Fetch operation
        op_rows = supa.client.table("operations").select("*").eq("id", op_id).execute().data or []
        if not op_rows:
            raise HTTPException(status_code=404, detail="operation not found")
        op_row = op_rows[0]

        payment_method = (op_row.get("payment_method") or "").lower()
        if payment_method != "credit":
            # If it's already cash/card, nothing to confirm
        ...
        amount = None
        try:
            amount = float((payload or {}).get("amount")) if (payload or {}).get("amount") is not None else None
        except Exception:
            amount = None
        ...
        # Determine already confirmed amount (sum of previous payment journal entries linked to this op)
        already_paid = 0.0
        try:
            prev = (
            ...
            for je in prev:
                # For payments, total field equals paid amount
                already_paid += float(je.get("total") or 0)
        except Exception as e:
            # If schema missing source/reference_id, we can't compute already paid; assume 0
        ...
        if op_type in ("sale", "service"):
            ...
            raise HTTPException(status_code=400, detail="unsupported operation type for payment confirmation")
        ...
        # Optional: if fully paid, update operation payment_method to cash and mark updated_at
        try:
        ...
        return {"success": True, "data": {"paid": round(pay_amount, 2), "remaining": round(max(0.0, remaining - pay_amount), 2)}}
