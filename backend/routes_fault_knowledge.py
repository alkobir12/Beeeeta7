...
async def list_faults(
    ...
    try:
        ...
        # Fallback to memory
        return {"success": True, "faults": fault_knowledge_memory[:limit], "source": "memory_fallback"}
    ...
    try:
        ...
        if use_supabase():
            try:
                ...
                return {"success": True, "fault": result.data[0] if result.data else fault_data, "source": "supabase"}
            except Exception as e:
                print(f"Supabase insert error: {e}")
                # Fallback to memory
                fault_knowledge_memory.append(fault_data)
                return {"success": True, "fault": fault_data, "source": "memory_fallback"}
        else:
            fault_knowledge_memory.append(fault_data)
...
async def search_similar_faults(
    ...
    try:
        if use_supabase():
            ...
            return {"success": True, "results": results, "count": len(results), "source": "supabase"}
        else:
            # Memory search
            results = []
            ...
            return {"success": True, "results": results[:10], "count": len(results), "source": "memory"}
...
async def delete_fault(fault_id: str):
    ...
    try:
        ...
        fault_knowledge_memory = [f for f in fault_knowledge_memory if f['id'] != fault_id]
