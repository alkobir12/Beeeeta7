...
async def get_engine_specs(engine_id: str):
    ...
    if not specs:
        raise HTTPException(
            status_code=404, detail="المحرك غير موجود / Engine not found"
        )
    return specs
...
async def get_generation_info(generation: str):
    ...
    if not info:
        raise HTTPException(
            status_code=404, detail="الجيل غير موجود / Generation not found"
        )
    return info
...
async def list_reports():
    ...
    docs = (
        await db.injector_reports.find({"type": "injector_report"})
        .sort("createdAt", -1)
        .to_list(length=100)
    )
    for d in docs:
