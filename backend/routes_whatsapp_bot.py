...
SOUND_SIGNATURES = {
    "normal": {
        ...
        "severity": "سليم ✅",
        "color": "#22c55e",
        "causes": ["صوت المكينة طبيعي"],
    },
    "knocking": {
        "name_ar": "طقطقة",
        "severity": "عالي ⚠️",
        "color": "#ef4444",
        "causes": ["بخاخات تالفة", "صمامات", "بيرنقات"],
    },
    "whistling": {
        "name_ar": "صفير",
        "severity": "متوسط ⚠️",
        "color": "#eab308",
        "causes": ["تسريب هواء", "تيربو"],
    },
    "rattling": {
        "name_ar": "خشخشة",
        "severity": "متوسط-عالي ⚠️",
        "color": "#f97316",
        "causes": ["سلسلة تايمنق"],
    },
    "grinding": {
        "name_ar": "احتكاك معدني",
        "severity": "خطير 🚨",
        "color": "#dc2626",
        "causes": ["بيرنقات تالفة", "مضخة زيت"],
    },
}
    ...
    async def download_media(self, media_url: str) -> bytes:
        async with httpx.AsyncClient(timeout=60.0) as client:
            response = await client.get(
                media_url, headers={"Authorization": f"App {Config.INFOBIP_API_KEY}"}
            )
...
class AudioAnalyzer:
    async def analyze(self, audio_data: bytes) -> dict:
        try:
            ...
            sharpness = (
                min(1.0, max(0.0, float(np.std(onset_env) / np.mean(onset_env)) / 2))
        ...
        if msg_type == "TEXT":
            ...
            if vin_match:
                ...
                if vin in CUSTOMER_CARS:
                    ...
                    parts_text = "\n".join(
                        [
                            f"• {p['name']}: {p['price_min']}-{p['price_max']} ريال"
                            for p in list(car["parts"].values())[:4]
                        ]
                    )
                    response = f"🚗 *سيارتك:*\n{car['model']} {car['year']}\n🔧 {car['engine']['code']}\n\n💰 *القطع:*\n{parts_text}\n\n📞 {WORKSHOP_INFO['phone']}"
            ...
            media_url = message_data.get("audio", {}).get(
                "url", ""
            ) or message_data.get("voice", {}).get("url", "")
            if media_url:
...
📞 للحجز: {WORKSHOP_INFO['phone']}"""
                    ...
                    await infobip.send_text(
                        sender, f"❌ فشل التحليل\n📞 {WORKSHOP_INFO['phone']}"
                    )
