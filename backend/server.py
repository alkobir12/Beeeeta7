...
def calculate_estimated_total(parts: List[Any]) -> float:
    """Calculates the estimated total from parts/services list."""
    total = 0.0
    ...
    try:
        ...
        vehicle_dict = {
            **vehicle_data.dict(),
            ...
            "estimatedTotal": calculate_estimated_total(vehicle_data.parts),
        }
...
async def get_vehicles():
    if DB_PROVIDER == "supabase":
        ...
        for r in rows:
            if r.get("status") is None:
                r["status"] = "diagnosis"
            parts = r.get("parts", [])
            r["estimatedTotal"] = calculate_estimated_total(parts)
    ...
    for v in vehicles:
        if v.get("status") is None:
            v["status"] = "diagnosis"
        parts = v.get("parts", [])
        v["estimatedTotal"] = calculate_estimated_total(parts)
