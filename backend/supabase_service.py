...
class SupabaseService:
    ...
    def vehicles_count_active(self) -> int:
        """Fetch count of active vehicles (not delivered or cancelled) efficiently."""
        if self.mock_mode:
            return 0
        try:
            # We fetch only the 'id' column and use head=True to get only the count
            res = (
                self.client.table("vehicles")
                .select("id", count="exact")
                .not_.in_("status", ["delivered", "cancelled"])
                .limit(0)  # Don't return any rows
                .execute()
            )
            return res.count if res.count is not None else 0
        except Exception as e:
            print(f"Supabase active vehicles count error: {e}")
            return 0
    ...
    def customers_count(self) -> int:
        """Fetch total customer count efficiently."""
        if self.mock_mode:
            return 0
        try:
            res = (
            ...
            print(f"Supabase customers count error: {e}")
            return 0
