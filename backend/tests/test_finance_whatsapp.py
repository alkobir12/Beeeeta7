...
class TestFinanceAPIs:
    ...
    def test_journal_entries_api(self):
        ...
        if len(data["data"]) > 0:
            ...
            if len(entry["lines"]) > 0:
                ...
                assert "debit" in line or "credit" in line, "Line should have debit or credit"
    ...
    def test_chart_of_accounts_api(self):
        ...
        if len(data["data"]) > 0:
            ...
            assert account["type"] in valid_types, f"Account type should be one of {valid_types}"
    ...
    def test_balance_sheet_api(self):
        ...
        liabilities_plus_equity = totals.get("liabilities_plus_equity", liabilities + equity)
        ...
        print(f"✅ Balance sheet API - Assets: {assets}, Liabilities: {liabilities}, Equity: {equity}")
...
class TestWhatsAppBotAPIs:
    ...
    def test_whatsapp_bot_stats(self):
        ...
        assert "total_messages" in data or "messages_count" in data, "Stats should have message count"
        assert "cars_count" in data, "Stats should have cars_count"
        
        print(f"✅ WhatsApp bot stats API - Cars: {data.get('cars_count', 0)}, Messages: {data.get('total_messages', data.get('messages_count', 0))}")
    ...
    def test_whatsapp_bot_workshop_info(self):
        ...
        print(f"✅ WhatsApp bot workshop info - Name: {data.get('name')}, Phone: {data.get('phone')}")
...
class TestHealthAndBasicAPIs:
    ...
    def test_stats_endpoint(self):
        ...
        data = response.json()
        assert "totalCustomers" in data or "activeVehicles" in data, "Stats should have customer or vehicle count"
        print(f"✅ Stats endpoint - Customers: {data.get('totalCustomers', 0)}, Active Vehicles: {data.get('activeVehicles', 0)}")
