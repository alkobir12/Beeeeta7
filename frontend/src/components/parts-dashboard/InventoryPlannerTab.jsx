...
export const InventoryPlannerTab = ({ architecture, loading = false }) => {
  ...
  if (loading) {
    return (
      ...
      <section className="glass-card p-4" data-testid="inventory-architecture-order-builder">
        <div className="flex flex-wrap items-center justify-between gap-2">
          <div>
            <h3 className="text-white font-semibold">طلبات الشراء عبر واتس اب</h3>
            <p className="text-xs text-slate-400">أنشئ طلبًا لكل مورد، ثم عاين الرسالة قبل الإرسال.</p>
          </div>
        </div>

        {!supplierKeys.length && (
          <p className="mt-4 text-sm text-slate-400" data-testid="inventory-architecture-order-empty">
            لم تتم إضافة أصناف للطلب بعد.
          </p>
        )}

        {!!supplierKeys.length && (
          <div className="mt-4">
            <div className="flex flex-wrap gap-2" data-testid="inventory-architecture-order-suppliers">
              {supplierKeys.map((supplier) => (
                <button
                  ...
                  data-testid={`inventory-architecture-order-supplier-${supplier}`}
                >
                  {supplier}
                </button>
              ))}
            ...
            {activeDraft && (
              <div className="mt-4 grid grid-cols-1 lg:grid-cols-2 gap-4">
                <div className="space-y-2" data-testid="inventory-architecture-order-items">
                  {activeDraft.items.map((item) => (
                    ...
                    >
                      <div className="flex items-center justify-between">
                        <p className="text-sm text-white">{item.part_name}</p>
                        <button
                          type="button"
                          onClick={() => removeOrderItem(activeDraft.supplier, item.part_id)}
                          className="text-xs text-rose-300 hover:text-rose-200"
                          data-testid={`inventory-architecture-order-remove-${item.part_id}`}
                        >
                          حذف
                        </button>
                      </div>
                      <div className="flex flex-wrap items-center gap-2 text-xs text-slate-300">
                        <span>الكمية المقترحة: {item.suggested_order_quantity}</span>
                        <span>الاستهلاك الشهري: {item.avg_monthly_usage}</span>
                      </div>
                      <div className="flex items-center gap-2">
                        <label className="text-xs text-slate-400">الكمية المطلوبة</label>
                        <input
                          type="number"
                          min="1"
                          value={item.quantity}
                ...
                <div className="rounded-2xl border border-white/10 bg-white/5 p-4">
                  <div className="flex items-center justify-between">
                    <h4 className="text-sm text-white font-semibold">معاينة الطلب قبل الإرسال</h4>
                    {activeDraft.phone ? (
                      <span className="text-xs text-slate-400">رقم المورد: {activeDraft.phone}</span>
                    ) : (
                      <span className="text-xs text-amber-300">لا يوجد رقم مورد مسجل</span>
                    )}
                  </div>
                  <textarea
                    className="mt-3 w-full h-40 rounded-lg border border-white/10 bg-slate-950/60 p-3 text-xs text-slate-200"
                    readOnly
                  ...
                  <div className="mt-3 flex flex-wrap items-center gap-2">
                    {whatsappUrl ? (
                      <a
                        ...
                        target="_blank"
                        rel="noreferrer"
                        className="rounded-lg bg-emerald-500/20 px-4 py-2 text-xs text-emerald-200 hover:bg-emerald-500/30"
                        data-testid="inventory-architecture-order-send-whatsapp"
                      >
                        إرسال عبر واتس اب
                      </a>
                    ) : (
                      <span className="rounded-lg bg-white/5 px-4 py-2 text-xs text-slate-400" data-testid="inventory-architecture-order-send-disabled">
                        أدخل رقم المورد لإرسال واتس اب
                      </span>
                    )}
                  </div>
                </div>
