...
// Optimization: Move stateless helper functions outside the component to prevent re-creation on each render
...
const getServiceTypeLabel = (items = []) => {
  if (!items || !Array.isArray(items)) return 'غير محدد';
  const names = items
...
const Dashboard = () => {
  ...
  // Optimization: Memoize configuration object that depends on translation hook
  ...
  // Optimization: Lazy load vehicle summary only when expanded to prevent N+1 API calls on mount
  useEffect(() => {
  ...
  const stats = useMemo(() => {
    ...
    // Optimization: Calculate all stats in a single O(N) pass instead of multiple filters
    ...
    dashboardVehicles.forEach(vehicle => {
      ...
      if (status === 'waiting_for_parts') waitingParts++;
      if (status === 'diagnosis') diagnosis++;
      if (status === 'delivering') delivering++;
      if (status === 'ready' || status === 'delivering') readyForHandover++;
  ...
  const filteredVehicles = useMemo(() => {
    // Optimization: Pre-calculate lowercase query outside the loop
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 sm:gap-4 md:gap-6">
          {filteredVehicles.length === 0 ? (
            ...
            filteredVehicles.map((vehicle) => {
              ...
              // Optimization: Use locally available data as fallback while lazy loading
              const serviceType = summary.serviceType || getServiceTypeLabel(vehicle.parts) || 'غير محدد';
              const estimatedTotal = summary.estimatedTotal ?? vehicle.estimatedTotal ?? (
                (vehicle.parts && Array.isArray(vehicle.parts))
                  ? vehicle.parts.reduce((sum, p) => sum + (Number(p.price || 0) * Number(p.quantity || 1)), 0)
                  : 0
              );
              return (
                <div
