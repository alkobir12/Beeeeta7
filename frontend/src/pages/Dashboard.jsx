...
const Dashboard = () => {
  ...
  const stats = useMemo(() => {
    ...
    dashboardVehicles.forEach((vehicle) => {
      const is_in_progress = inProgressStatuses.has(vehicle.status);
      if (is_in_progress) {
        inProgress++;
        const techKey = String(vehicle.technicianId || vehicle.technicianName || '').trim();
        if (techKey) busyTechnicianKeys.add(techKey);
      }
      if (vehicle.status === 'ready') ready++;
      if (vehicle.status === 'waiting_for_parts') waitingParts++;
      if (vehicle.status === 'diagnosis') diagnosis++;
      if (vehicle.status === 'delivering') delivering++;
      if (['ready', 'delivering'].includes(vehicle.status)) readyForHandover++;
