...
const KnowledgeBase = () => {
  ...
  const handleSearch = async () => {
    ...
    try {
      ...
      if (isDTCSearch || searchMode === 'dtc') {
        ...
        setDtcCards([]);
        toast({ title: '✅ اكتمل البحث', description: `وجدنا ${res.data?.count || 0} نتيجة من ${res.data?.totalMatches || 0} مطابقة` });
      }
    } catch (e) {
  ...
  const handleUpload = async (e) => {
    ...
    try {
      ...
      const res = await axios.post(`${API_URL}/ai/kb/upload-and-analyze`, formData, { headers: { 'Content-Type': 'multipart/form-data' } });
      toast({ title: '✅ تم الرفع والتحليل', description: `الملف: ${res.data.filename} - استُخرج ${res.data.extractedLength} حرف` });
      loadDocuments();
    } catch (e) {
              ...
              <Card className="card-godaddy mb-6">
                ...
                <CardContent className="p-6">
                  <div className="flex gap-3 mb-4">
                    ...
                    <Button onClick={handleSearch} disabled={loading} className="btn-godaddy-primary px-8">
                  ...
                  <div className="mb-6">
                    <Input value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} onKeyPress={(e) => e.key === 'Enter' && handleSearch()} placeholder="ابحث: كهرباء، محرك، P0087، فرامل، تويوتا..." className="input-godaddy flex-1 text-lg" />
                    <Button onClick={handleSearch} disabled={loading} className="btn-godaddy-primary px-8">
                      {loading ? '⏳ جاري البحث...' : 'بحث'}
                    </Button>
                  </div>

                  <div className="mb-4 text-sm text-godaddy-gray">💡 نصيحة: ابحث عن كود العطل (مثال: P0087) أو كلمة مفتاحية (كهرباء، محرك، SCV)</div>
                        ...
                        <Card key={idx} className="border-2 border-amber-200 hover:border-godaddy-green hover:shadow-lg transition-all">
                          <CardContent className="p-6">
                            ...
                            <div className="flex items-start justify-between mb-4">
                              <div>
                                <div className="flex items-center gap-3 mb-2">
                                  <span className="px-4 py-2 bg-red-600 text-white font-mono font-bold text-lg rounded-lg">{card.code}</span>
                                  <span className="px-3 py-1 bg-blue-100 text-blue-700 rounded text-sm">{card.vehicle}</span>
                                </div>
                                <h3 className="font-bold text-xl text-godaddy-black mb-1">{card.name}</h3>
                                <p className="text-xs text-godaddy-gray">📄 المصدر: {card.source}</p>
                              </div>
                            </div>
                            {/* Causes */}
                            {card.causes && card.causes.length > 0 && (
                              <div className="mb-4 p-4 bg-red-50 border border-red-200 rounded-lg">
                                <h4 className="font-semibold text-red-900 mb-2 flex items-center gap-2">⚠️ الأسباب المحتملة:</h4>
                                <ul className="space-y-1">
                                  {card.causes.map((cause, i) => (
                                    <li key={i} className="text-sm text-red-800">• {cause}</li>
                                  ))}
                                </ul>
                              </div>
                            )}
                            ...
                            {card.fixes && card.fixes.length > 0 && (
                              <div className="mb-4 p-4 bg-green-50 border border-green-200 rounded-lg">
                                <h4 className="font-semibold text-green-900 mb-2 flex items-center gap-2">🔧 طرق الإصلاح:</h4>
                                <ul className="space-y-1">
                                  {card.fixes.map((fix, i) => (
                                    <li key={i} className="text-sm text-green-800">✓ {fix}</li>
                                  ))}
                                </ul>
                              </div>
                            )}
                            ...
                            {card.related && card.related.length > 0 && (
                              <div className="p-4 bg-blue-50 border border-blue-200 rounded-lg mb-4">
                                <h4 className="font-semibold text-blue-900 mb-2">🔗 أكواد ذات صلة:</h4>
                                <div className="flex gap-2 flex-wrap">
                                  {card.related.map((code, i) => (
                                    <button key={i} onClick={() => { setSearchQuery(code); handleSearch(); }} className="px-3 py-1 bg-blue-600 text-white rounded font-mono text-sm hover:bg-blue-700">{code}</button>
                                  ))}
                                </div>
                              </div>
                            )}
                            ...
                            <div className="p-4 bg-purple-50 border-2 border-purple-300 rounded-lg mb-4">
                              <h4 className="font-semibold text-purple-900 mb-2 flex items-center gap-2">📚 المرجع الفني (Repair Manual):</h4>
                              <div className="flex items-center justify-between">
                                <div>
                                  <p className="text-sm text-purple-800 font-medium">🚗 {card.vehicle}</p>
                                  <p className="text-xs text-purple-700 mt-1">📄 {card.repairManual || card.source}</p>
                                </div>
                                ...
                                }}> <BookOpen size={16} className="ml-1" /> افتح المرجع</Button>
                              </div>
                            </div>
                            {/* Read Full Document */}
                            <div className="pt-4 border-t">
                              <Button size="sm" variant="outline" onClick={() => {
                                ...
                                <Eye size={16} className="ml-1" /> اقرأ المستند الكامل
                              </Button>
                            </div>
                          </CardContent>
                        </Card>
                          ...
                          <CardContent className="p-5">
                            <div className="flex items-start justify-between gap-4">
                              ...
                              <Button size="sm" variant="outline" className="flex-shrink-0">
                                <Eye size={16} className="ml-1" /> قراءة
                              </Button>
                            </div>
                          </CardContent>
                        </Card>
                  ...
                  <Card key={idx} className="card-godaddy hover:border-godaddy-green cursor-pointer" onClick={() => openReader(doc)}>
                    <CardContent className="p-5">
                      ...
                      <div className="flex items-center gap-2 mb-3">
                        ...
                        {doc.subcategory && (
                          <span className="px-2 py-1 bg-blue-50 text-blue-700 rounded text-xs">{doc.subcategory}</span>
                        )}
                      </div>
                      {doc.summary && (
                      ...
                      <Button size="sm" variant="outline" className="w-full">
                        <Eye size={16} className="ml-1" /> قراءة المستند
                      </Button>
                    </CardContent>
                  </Card>
                ))}
                ...
                <CardContent className="p-6">
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                    <div>
                      ...
                      <select className="w-full border-2 rounded-lg p-3" onChange={(e) => setFile1(documents.find(d => d.id === e.target.value))}>
                    ...
                    <div>
                      ...
                      <select className="w-full border-2 rounded-lg p-3" onChange={(e) => setFile2(documents.find(d => d.id === e.target.value))}>
                        <option value="">اختر مستند...</option>
                        {documents.map(doc => (<option key={doc.id} value={doc.id}>{doc.filename}</option>))}
                      </select>
                    </div>
                  ...
                  <Button onClick={async () => {
                    if (!file1 || !file2) { toast({ title: 'اختر ملفين', variant: 'destructive' }); return; }
                    try {
                      setLoading(true);
                      const res = await axios.post(`${API_URL}/ai/kb/compare-files`, { file1_id: file1.id, file2_id: file2.id });
                      setComparisonResult(res.data);
                      toast({ title: '✅ اكتملت المقارنة' });
                    } catch (e) {
                      toast({ title: 'خطأ في المقارنة', variant: 'destructive' });
                    } finally { setLoading(false); }
                  }} disabled={!file1 || !file2 || loading} className="btn-godaddy-primary w-full">
                  ...
                  {comparisonResult && (
                    <Card className="mt-6 border-2 border-purple-300">
                      <CardContent className="p-6">
                        ...
                        <pre className="whitespace-pre-wrap font-sans text-sm leading-relaxed">{comparisonResult.comparison}</pre>
                      </CardContent>
                    </Card>
                  )}
                </CardContent>
                  ...
                  <div className="space-y-4">
                    <div className="border-2 border-dashed border-godaddy-gray-light rounded-lg p-8 text-center bg-gray-50 hover:bg-gray-100 transition-colors">
                      ...
                      <Label htmlFor="file-upload" className="cursor-pointer">
                        <p className="text-lg font-semibold text-godaddy-black mb-2">اضغط لاختيار ملف أو اسحبه هنا</p>
                        <p className="text-sm text-godaddy-gray">PDF, DOCX, TXT - حتى 50 MB</p>
                      </Label>
                      <Input id="file-upload" type="file" accept=".pdf,.docx,.doc,.txt" onChange={handleUpload} className="hidden" disabled={uploadProgress} />
                    </div>
                    {uploadProgress && (
                  ...
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      ...
                      <p className="text-sm text-gray-300 flex items-center gap-3 mt-1">
                        ...
                        {selectedDoc.vehicle && (<><span>•</span><span>🚗 {selectedDoc.vehicle}</span></>)}
                      </p>
                    </div>
                  </div>
                  <Button variant="ghost" onClick={() => setShowReader(false)} className="text-white hover:bg-gray-700 text-2xl px-4">✕</Button>
                </div>
                {/* Reader Content */}
                    ...
                    {selectedDoc.summary && (
                      <Card className="mb-6 border-2 border-godaddy-green shadow-lg">
                        <CardHeader className="bg-gradient-to-l from-green-50 to-white">
                          <CardTitle className="text-xl flex items-center gap-2"><Zap className="text-godaddy-green" size={24} />الملخص الذكي بالذكاء الاصطناعي</CardTitle>
                        </CardHeader>
                        <CardContent className="p-6 bg-white">
                          <div className="prose prose-lg max-w-none text-godaddy-black leading-relaxed">
                            <pre className="whitespace-pre-wrap font-sans text-base leading-loose">{selectedDoc.summary}</pre>
                          </div>
                        </CardContent>
                      </Card>
                    )}
                          ...
                          <div className="grid grid-cols-1 gap-3">
                            {selectedDoc.keyPoints.map((point, i) => (
                              <div key={i} className="flex items-start gap-3 p-4 bg-blue-50 rounded-lg border border-blue-200">
                                <span className="flex-shrink-0 w-8 h-8 rounded-full bg-godaddy-green text-white flex items-center justify-center font-bold">{i + 1}</span>
                    ...
                    {selectedDoc.content && (
                      <Card className="mb-6 shadow-lg">
                        <CardHeader className="bg-gradient-to-l from-amber-50 to-white">
                          <CardTitle className="text-xl flex items-center gap-2"><FileText className="text-amber-600" size={24} />قراءة المستند الكامل - عرض احترافي</CardTitle>
                        </CardHeader>
                        <CardContent className="p-8 bg-white">
                          <div className="bg-white border-2 border-gray-200 rounded-xl p-10 shadow-inner" style={{fontFamily:"'Amiri', 'Traditional Arabic', 'Arial', sans-serif",fontSize:'16px',lineHeight:'2.2',color:'#1a202c'}}>
                            <pre className="whitespace-pre-wrap" style={{fontFamily:'inherit',fontSize:'inherit',lineHeight:'inherit',color:'inherit',direction:'rtl',textAlign:'justify'}}>{selectedDoc.content}</pre>
                          </div>
                          <div className="mt-6 pt-6 border-t flex items-center justify-between text-sm text-gray-600">
                            ...
                            {selectedDoc.accessCount && (<span>👁️ عدد القراءات: {selectedDoc.accessCount}</span>)}
                          </div>
                        </CardContent>
                      </Card>
                    )}
                        ...
                        <CardContent className="p-6">
                          <div className="flex flex-wrap gap-2">
                            {selectedDoc.keywords.map((kw, i) => (
                              <span key={i} className="px-4 py-2 bg-godaddy-green text-white rounded-full font-semibold text-sm">#{kw}</span>
                            ))}
                          </div>
                        </CardContent>
                      </Card>
