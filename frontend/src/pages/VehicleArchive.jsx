    ...
    <Layout>
      <div className="max-w-7xl mx-auto space-y-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          ...
          <div className="apple-card p-5 flex items-center justify-between">
            <div><p className="text-sm text-gray-500 mb-1">تم التسليم</p><p className="text-2xl font-bold text-green-600">{vehicles.filter(v => v.status === 'ready').length}</p></div>
            <div className="w-10 h-10 rounded-full bg-green-50 flex items-center justify-center text-green-600"><FileText size={20} /></div>
          </div>
          <div className="apple-card p-5 flex items-center justify-between">
            <div><p className="text-sm text-gray-500 mb-1">قيد العمل</p><p className="text-2xl font-bold text-orange-600">{vehicles.filter(v => v.status !== 'ready').length}</p></div>
        ...
        <div className="apple-card p-4 flex flex-col md:flex-row gap-4">
          ...
          <div className="flex gap-2 overflow-x-auto pb-2 md:pb-0">
            {['all', 'diagnosis', 'quotation', 'repair', 'ready'].map(status => (
