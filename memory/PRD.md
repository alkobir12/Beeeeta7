...
- في صفحة `VehicleDetails.jsx`، تمت إضافة المكوّن الجديد `RakanLinkedPartPicker`:
  - يظهر تلقائياً عند اختيار `itemType=supplier` واسم المورد يحتوي على «راكان» (أو rakan).
  - يحتوي على قائمة منسدلة فيها قطع المخزون التي مُورّدها راكان (مع fallback لكل القطع إذا لا توجد قطع موسومة).
  - زر «إدخال يدوي» لكتابة اسم قطعة جديدة غير موجودة في المخزون.
  - الاختيار يُحفظ ضمن نفس بند المورد كحقلَين: `linkedPart` و `linkedPartManualEntry` — لا يُنشأ بند منفصل.
  - تنبيه ظاهر للمستخدم: «هذا البند يُسجَّل في تحليلات راكان فقط — لا يدخل في دفتر اليومية. تجنّب تكرار نفس القطعة عبر نقطة البيع أو شاشة العمليات.»
- المكوّن مدمج في النوعين: row table view (`VisitItemRow`) و الـ card view.

### Rakan Independence + Supplier Movements (Final, 25 Apr 2026)
- **راكان وحدة مستقلة عن الورشة**: `_build_operation_journal_entry` يعيد `None` لأي عملية تُكتشف كـ Rakan → **لا يُنشأ قيد محاسبي على الإطلاق** للعمليات الجديدة. تحليلات راكان تتم في تبويب «تحليلات قطع راكان» الموجود في لوحة تحكم القطع.
- **القيود الموجودة سابقاً تبقى كما هي** بناءً على طلب المستخدم (option ج) — لا حذف ولا إخفاء.
- **عمليات بيع قطع غير راكان** بدون خدمات → `return None` (لا قيد، فقط أرشيف operations + حركة مورد).
- **سجل حركات الموردين** (`server.py::_augment_supplier_movements_from_journal`):
  - يقرأ من `journal_entries` ويطابق اسم المورد كـ substring مع `account_name` للسطور.
  - راكان: ٨١٦ ر.س. عبر 4 حركات | ساسكو: ٣٥٠ ر.س. عبر حركة واحدة | باقي الموردين 0 (طبيعي).
  - يستخدم cache من routes_finance لتفادي الجلب المكرر.
