...
### Rakan Routing + Supplier Movements Restoration (25 Apr 2026)
- **توجيه عمليات راكان لحساب 044** (`routes_extended.py::_build_operation_journal_entry`):
  - عمليات بيع راكان (sale) → دائن **044** (قطع غيار راكان) بدلاً من 4100/الإيراد العام.
  - عمليات شراء/مصروف راكان (purchase/expense) → مدين **044** بدلاً من 6100.
  - النتيجة: حساب 044 يصبح حساب وحدة أعمال (P&L net) لراكان — صافيه = ربح/خسارة وحدة راكان.
  - الكشف يستخدم نفس `is_rakan_operation` الحالي (selected_meta.is_rakan أو scope/source/notes/business_unit يحتوي «rakan_parts»).
- **عمليات بيع قطع غير راكان لا تنشئ قيود محاسبية**:
  - إذا كانت العملية = sale + كل items هي parts (لا توجد خدمات) + ليست راكان → ترجع `None` (لا قيد) لكن العملية تبقى محفوظة في جدول operations.
  - الإيراد لا يدخل قائمة الدخل، ويظهر فقط في سجل المورد كحركة.
- **إصلاح سجل حركات الموردين** (`server.py::_augment_supplier_movements_from_journal`):
  - مشكلة سابقة: كل الـ 13 مورد كانوا يعرضون 0 حركات.
  - الإصلاح: قراءة من `journal_entries` ومطابقة اسم المورد كـ substring مع `account_name` للسطور (مع اقتران أطول الأسماء أولاً لتفادي الالتباس بين «الزايدي» و«الزايدي ليات»).
  - يلتقط الآن الحسابات الفرعية المباشرة (`مورد - <name>`) بالإضافة إلى الحسابات الموضوعية (`مصروفات شخصيه راكان`, `مصروفات بنزين راكان` ...).
  - استعمال cache من routes_finance لتفادي الجلب المكرر من Supabase.
- **النتائج:**
  - راكان: **مدين 816 ر.س.** عبر 4 حركات (مصروفات شخصية + بنزين × عدة مرات).
  - ساسكو: **مدين 350 ر.س.** (سداد بنزين غسيل).
  - باقي الموردين: 0 (لا توجد قيود مرتبطة بهم — صحيح).
...
- **تنظيف الحسابات المكررة**: حذف 32 حساب مكرر/تجريبي من Supabase + ملف aliases (211 → 179 حساباً):
  ...
  - مكررات موردين (14): الحصان، الدرويش، الدريويش، الراكضي، الزايدي، الزايدي ليات، العبدالرحيم، القرعاوي، المطعم، النهدي، تشليح، عبدالرحيم، مخرطة العوفي (×2), ميزان
  - تباينات إملائية مدمجة (3): عبدالرحيم ⇄ العبدالرحيم، دانية ⇄ دانيه، مخرطه ⇄ المخرطه
  - حسابات تجريبية (4): «مورد اختبار T1», «TEST_مورد_اختبار», «عميل ملف بحث», «عميل ربط ملف»
  - مكرر أصول (1): «نقاط بيع» الفرعي (210) ← الأساسي (006)
  - جميع الحسابات المحذوفة كانت برصيد 0 وبدون حركات (آمن تماماً).
- **إعادة تنسيق كرت «صافي الدخل» (`ComprehensiveFinancial.jsx`)**:
  - استبدال قائمة النقاط المكدسة `• text` بجدول Key-Value منظّم مع فواصل خفيفة.
  - `details` أصبحت `{label, value}[]` بدلاً من `string[]`.
  - `subtitle` تدعم الآن Array of chips (الهامش | إيراد نقد | إيراد بنك) بدون `•` متلاصقة.
  - استخدام `tabular-nums` للأرقام لمحاذاة أنيقة.
- **إعادة تنسيق شريط ملخص دليل الحسابات (`ChartOfAccountsLiquid.jsx`)**:
  - تقسيم السطر الواحد إلى **5 بطاقات** (Grid): الأصول / الخصوم / الإيرادات / المصروفات+المشتريات / الصافي — لكل بطاقة حد ملوّن (emerald/rose/sky/amber/cyan).
  - تقسيم شريط البنك/النقد/POS/الإيراد إلى **4 بطاقات** منفصلة مع كود الحساب ملوّناً.
...
- `ComprehensiveFinancial.jsx`:
  - إزالة الإشارات النصية للأكواد القديمة من الأزرار والشرح.
  - الاعتماد على اختيار حساب الإيراد الجذري ديناميكيًا من الدليل بدل `4000` الثابت.
  - الحفاظ على التحليل المالي بدون عرض أكواد قديمة للمستخدم.
- `DebtFollowUp.jsx`:
  - إلغاء ربط السيولة بنمط أكواد قديم، والاعتماد على تصنيف الأسماء (نقد/بنك).
  - تحديث النصوص لإزالة مرجع 1101/1102.
- `JournalEntries.jsx`:
  - تقوية تحويل الأكواد القديمة إلى الجديدة في العرض.
  - منع إظهار legacy code إذا لم يكتمل تحميل mapping.
- `ChartOfAccountsLiquid.jsx`:
  - إلغاء الاعتماد على `legacy_code` في العرض المباشر، واستخدام اكتشاف الحسابات بالاسم (بنك/نقد/نقاط بيع).
- `ARReceivablesTab.jsx`:
  - تحديث اسم ملف التصدير لإزالة مرجع `1103`.

### Testing
- تقرير تحقق شامل: `/app/test_reports/iteration_163.json`
  - Backend: 10/10 PASS
  - Frontend: PASS
  - النتيجة: ✅ لا تظهر الأكواد القديمة في الصفحات المخصصة، وتم اعتماد الأكواد التسلسلية الجديدة.
...
- دليل الحسابات:
  ...
  - إصلاح التوسيع: أصبح فتح الحسابات الفرعية يعمل عند الضغط على **الصف كاملًا** (وليس أيقونة صغيرة فقط).
  - على الجوال: الحسابات ذات الفروع تتوسع داخل الشجرة، والحسابات بدون فروع تفتح Bottom Sheet للتفاصيل.

### Testing
- `/app/test_reports/iteration_159.json`
  - Collapse/Expand التدقيق: PASS
  - ظهور/اختفاء ملاحظة الطي: PASS
  - توسيع الفروع بالضغط على الصف: PASS (Code + Playwright checks)

### تعديل عاجل حسب ملاحظة المستخدم: الترقيم عبر كود الحساب نفسه + حل تكرار/تعطل الصفحة (23 Apr 2026)
- تم إلغاء الاعتماد على `display_code` في الواجهة، وأصبح الترقيم فعليًا على حقل `code` نفسه (001، 002...).
- `POST /api/accounts/reindex-display-codes` أصبح يعيد ترقيم **الكود الفعلي** للحسابات، مع حفظ `legacy_code` داخليًا لضمان توافق القيود القديمة.
- `GET /api/accounts/tree` يعيد الآن:
  - `code` = الكود الجديد المتسلسل
  - `legacy_code` = الكود القديم للتوافق الداخلي
- تم تحديث الواجهة (`ChartOfAccountsLiquid.jsx`) لإزالة حقل/عرض الترقيم المنفصل والاكتفاء بكود الحساب.
- تم التحقق من استقرار الصفحة (عدم تكرار مكسور) + عدم كسر APIs المحاسبية بعد التغيير.

### Testing
- تقرير اختبار: `/app/test_reports/iteration_158.json`
- Backend: 14/14 PASS (100%).
- Frontend: PASS لهيكل الصفحة وعناصرها؛ ملاحظة Playwright عن network transient ليست خللًا منطقيًا في الكود.

### تحسين دليل الحسابات + سياسة البنك/الكاش + تدقيق التطابق (23 Apr 2026)
- Frontend (`ChartOfAccountsLiquid.jsx`) على نفس المسار `/accounting/chart-of-accounts`:
  - تحسين تصفح شجرة الحسابات بشكل واضح ومتجاوب لجميع الأجهزة.
  - إضافة أزرار تشغيل مباشرة:
    - `إعادة ترقيم 001`
    - `تطبيق سياسة البنك/الكاش`
    - `تحديث تدقيق التطابق`
  - إضافة شريط ملخص مخصص: البنك (1102) / النقد (1101) / نقاط بيع (1104) / إجمالي الإيراد.
  - إضافة لوحة تدقيق تطابق المبالغ مع جدول تفصيلي (الرصيد، الرصيد المتوقع، الفرق، الحالة).

- Backend (`routes_extended.py`):
  - إضافة endpoint فعلي لإعادة ترقيم الحسابات: `POST /api/accounts/reindex-display-codes` (يحفظ `display_code` متسلسل يبدأ من 001).
  - إضافة endpoint لتقرير التطابق: `GET /api/accounts/reconciliation-report`.
  - تحديث `GET /api/accounts/tree` ليحسب الرصيد من القيود الفعلية (debit/credit) لضمان التطابق.
  - تضمين `display_code` في البحث والتصدير.
  - تحديث `POST /api/accounts/init-defaults` لإضافة حساب `1104 نقاط بيع` كفرعي تحت البنك.

- Backend (`routes_finance.py`):
  - إضافة endpoint سياسة المالك:
    - `POST /api/finance/reports/apply-bank-revenue-policy`
    - وظائفه: تحويل 1101 -> 1102 تاريخيًا، تحديث عمليات البيع/الخدمة النقدية إلى bank، وضمان وجود POS 1104 تحت 1102.

- تطبيق فعلي على Preview وفق طلب المستخدم:
  - تم تشغيل إعادة الترقيم فعليًا.
  - تم تطبيق سياسة البنك/الكاش فعليًا.
  - تم إنشاء POS وربطه تحت حساب البنك.

### Testing
- تقرير اختبار شامل: `/app/test_reports/iteration_157.json`.
- النتيجة: Backend 100% + Frontend 100%.
- تحقق مؤكد من المطلوب:
  - ترقيم الحسابات يبدأ من 001.
  - cash 1101 = 0 بعد السياسة.
  - bank 1102 يستقبل الإيرادات التاريخية.
  - reconciliation: 209/209 matched.
...
- تم معالجة السبب الجذري لعدم ظهور نتائج البحث في `NewVehicle`:
  ...
  - إضافة فهرس مركبات فعلي (`vehicleDirectory`) لاستخراج اللوحات وربطها بالعملاء.
  - دعم البحث الذكي بالحرف/الرقم الأول + الجزئي + الكامل + الأرقام العربية (`٠٠٠`).
- تم معالجة السبب الجذري لأرقام كرت الدخل الصفرية في `ComprehensiveFinancial`:
  - تعديل المدة الافتراضية إلى شامل تاريخي (`startDate = 2000-01-01`) بدل نافذة قصيرة قد تُظهر صفرًا.
  - تعزيز fallback لتفصيل نقد/بنك/آجل وأرصدة 1101/1102 من مصادر summary/components عند نقص الحقول المباشرة.

### Testing
- تقرير testing agent: `/app/test_reports/iteration_153.json`
  - Frontend: **100% PASS (12/12)**
- النتائج المؤكدة:
  - البحث باللوحة يعمل: `0`, `000`, `0000`, `٠٠٠` + عرض العميل واللوحة.
  - كرت الدخل يعرض صافي الدخل/الإيراد/المصروفات بأرقام صحيحة.
  - تفاصيل الكرت تعرض حساب البنك 1102 وقيم تفصيل الدفع بشكل صحيح.

### استقبال مركبة: بحث العملاء بالاسم/الجوال/رقم الملف/رقم اللوحة + تحسين الأداء (20 Apr 2026)
- تم تطوير حقل البحث عن عميل في صفحة `NewVehicle` ليشمل:
  - الاسم
  - رقم الجوال
  - رقم الملف
  - رقم اللوحة (بحث جزئي وكامل)
- النتائج الآن تعرض العميل + رقم اللوحة عند توفرها (🚘 plate).
- تحسين الأداء في البحث:
  - Debounce بقيمة `220ms`.
  - Local cache (`customerDirectory`) بدل الاعتماد على طلب API مع كل ضغطة.
  - Remote fallback باستخدام `archiveSearch` لالتقاط تطابقات إضافية.
  - دمج النتائج وإزالة التكرار مع سقف نتائج واضح.
- دعم ذكي إضافي تم اعتماده:
  - تطبيع الأرقام العربية/الفارسية إلى إنجليزية تلقائيًا (`٠١٢...` ⟶ `012...`).
  - تطبيع/ترجمة الحروف العربية-الإنجليزية في رقم اللوحة عبر `normalizePlateCanonical`.
  - إصلاح دعم البحث بأول حرف/رقم للوحة (Single-character plate search).

### Testing
- تقرير testing agent: `/app/test_reports/iteration_149.json`
  - Frontend: **100% PASS**
- تم التحقق من:
  - البحث باللوحة جزئيًا (`000`) وكاملًا (`0000`).
  - ظهور اللوحة داخل النتائج مع العميل.
  - تحسين الأداء (debounce + cache + fallback) بدون regressions.

### Verification Retest
- تقرير إعادة اختبار بعد إصلاح أول حرف/رقم: `/app/test_reports/iteration_151.json`
  - Frontend: **100% PASS (8/8)**
  - تم تأكيد:
    - البحث بحرف/رقم واحد (`0`) يعمل ويظهر النتائج.
    - البحث بالأرقام العربية (`٠٠٠`) يعمل.
    - عدم كسر البحث بالاسم/الجوال.
...
- تم تنفيذ وضع تحرير كامل من الأرشيف بدون تغيير الواجهة الأساسية:
  ...
  - داخل `VehicleDetails` يظهر Banner واضح: "وضع تحرير الأرشيف مفعّل".
  - تفعيل تعديل بيانات المركبة + العميل مباشرة.
  - دعم تعديل الزيارة والبنود بالكامل (إضافة/تعديل/حفظ/إغلاق/إعادة فتح).
  - للزيارات المغلقة في وضع الأرشيف: زر إعادة فتح للتعديل بصياغة أوضح.
- تمت إضافة **سجل تعديلات الأرشيف** في صفحة `/archive`:
  - Panel جديد يعرض آخر عمليات التحرير (نوع العملية + وقت + رقم اللوحة/الملف).
  - يتم تسجيل أحداث التعديل من ملف المركبة (مركبة/عميل/زيارة/حذف زيارة/إنشاء زيارة).
- تحسين جودة النماذج:
  - تطبيع بنود الزيارة عند التحميل لمنع تحذير `input value null`.

### Testing
- تقرير testing agent: `/app/test_reports/iteration_142.json`
  - Frontend: **100% PASS**
  - التحقق شمل: التنقل من الأرشيف، ظهور وضع التحرير، تعديل بيانات المركبة/العميل، تعديل البنود والزيارات، إعادة فتح الزيارة المغلقة، وسجل التعديلات.
...
### Hotreload/UX Simplification Pass (Iteration 134 - 17 Apr 2026)
- تم تبسيط toolbar لتقليل التعقيد البصري:
  - الأدوات الأساسية فقط تظهر افتراضيًا.
  - الأدوات المتقدمة تظهر عند الضغط على `أدوات +`.
- تم تثبيت سلوك الجوال:
  - لوحة الخصائص تبدأ `mobile-collapsed` افتراضيًا.
  - فتح/إغلاق عبر زر واضح دون تداخل دائم.
- تم تقليل الإحساس بالتحديث اللاإرادي:
  - التحكم في live sync أصبح يدويًا من المستخدم.
  - تقليل تكرار تحديثات/إعادة تطبيق المعاينة.
- التحقق النهائي (testing agent): `/app/test_reports/iteration_134.json`
  - جميع البنود PASS (Frontend 100%).
...
- تم تنفيذ مجموعة إصلاحات مباشرة على صفحة العمليات والمزامنة من ملف المركبة:
  ...
  - ضمان ترتيب العمليات **الأحدث أولًا** في الواجهة، مع تحسين ترتيب fallback في memory mode backend
  - تحسين تجربة تحميل العمليات عبر:
    - `initialData` من الكاش المحلي عند توفره
    - زيادة `staleTime`
    - تقليل تأخير تفعيل البيانات المؤجلة
- تم إصلاح منطق مزامنة الزيارة إلى عملية مالية في:
  - `/app/backend/visit_sync.py`
  - وأصبح يدعم:
    - قراءة `payments` من `notes`
    - استنتاج `payment_method` الصحيح من التحويل/البطاقة/النقد
    - تمرير `visit_id`
    - ضبط `scope/source/business_unit`
    - حفظ `payment_status`
- تم التأكد أن منطق القيد المالي يوجّه:
  - `cash` → **1101 النقد**
  - `transfer/card` → **1102 البنك**
- نتيجة الاختبار الرسمية: `iteration_132.json`
  - Frontend + backend: **PASS**
  - تم تأكيد:
    - تبويب الورشة هو الافتراضي
    - العمليات مرتبة بالأحدث أولًا
    - منطق payment method/journal mapping صحيح
