#!/usr/bin/env python3
"""
Operations Scope Testing Script - Final Version
اختبار ميزة (نوع العملية: مركبة / ورشة عامة) بعد التعديلات الأخيرة

Tests operations scope inference based on vehicleId presence.
"""
...
BACKEND_URL = "https://visit-notify-2.preview.emergentagent.com/api"
...
def test_operations_scope_final():
    """Test operations scope inference with real vehicle"""
    print("\n" + "="*80)
    ...
    if created_vehicle_id:
        try:
            ...
            if response.status_code == 200:
                ...
                if data.get('vehicleId') == created_vehicle_id:
                    print("   ✓ VehicleId field correctly preserved")
                else:
                    print(f"   ⚠ VehicleId field issue: expected '{created_vehicle_id}', got '{data.get('vehicleId')}'")
            else:
        ...
        log_test("Create vehicle operation", False, "Skipped - no test vehicle created")
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            if vehicle_op_found:
                ...
                if vehicle_op_found.get('vehicleId') == created_vehicle_id:
                    print("   ✓ Vehicle operation has correct vehicleId")
                else:
                    print(f"   ❌ Vehicle operation vehicleId issue: expected '{created_vehicle_id}', got '{vehicle_op_found.get('vehicleId')}'")
            elif vehicle_op_id:
    ...
    # Test 4: GET /api/operations?vehicle_id={created_vehicle_id} (verify filtering still works)
    if created_vehicle_id:
        print(f"\n[4] Testing GET /api/operations?vehicle_id={created_vehicle_id} (Filter by vehicle)")
        try:
            ...
            if response.status_code == 200:
                ...
                log_test("Filter operations by vehicle_id", True, 
                        f"Retrieved {len(filtered_operations)} operations for vehicle '{created_vehicle_id}'")
                ...
                if vehicle_op_in_filter:
                    ...
                    if vehicle_op_in_filter.get('vehicleId') == created_vehicle_id:
                        ...
                        print(f"   ❌ Filtered operation vehicleId issue: expected '{created_vehicle_id}', got '{vehicle_op_in_filter.get('vehicleId')}'")
                elif vehicle_op_id:
        ...
        log_test("Filter operations by vehicle_id", False, "Skipped - no test vehicle created")
    
    # Cleanup: Delete test vehicle
    if created_vehicle_id:
        print("\n[Cleanup] Deleting test vehicle")
        try:
            response = requests.delete(f"{BACKEND_URL}/vehicles/{created_vehicle_id}", timeout=15)
            if response.status_code == 200:
                print("   ✓ Test vehicle deleted successfully")
            else:
                print(f"   ⚠ Failed to delete test vehicle: {response.status_code}")
        except Exception as e:
            print(f"   ⚠ Error deleting test vehicle: {str(e)}")

def main():
    """Main test execution"""
    print("OPERATIONS SCOPE TESTING - FINAL VERSION")
    print("اختبار ميزة (نوع العملية: مركبة / ورشة عامة)")
    
    # Run the operations scope tests
    test_operations_scope_final()
